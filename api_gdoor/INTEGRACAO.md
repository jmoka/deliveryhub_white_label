...
4. `server_delivery` nunca chama o `api_gdoor` diretamente — só faz `INSERT` numa fila (`gdoor_jobs`), que o poller consome. **Nenhum gatilho automático dispara isso hoje** (removido em 2026-08-30 pros três fluxos — delivery, comanda de salão e balcão — ver Gotcha "prevenda enviada mas não aparecia" abaixo): a única forma de enfileirar uma venda é o botão manual "Enviar para GDOOR", disponível na tela de pedido/comanda já concluída e na lista de Pedidos da Sessão (`/restaurante/sessao`). Ambos chamam `GdoorService.enviarManual` via `POST /restaurante/gdoor/pedido/:id/enviar`. `GdoorService.criarJob()` mantém dedup (ignora se já existe job pendente/processado pro mesmo pedido) — relevante caso o botão seja clicado mais de uma vez.
...
### Gotcha: prevenda automática de comanda "processada" mas não aparecia na tela do GDOOR (2026-08-30)

Depois de corrigir o gatilho automático (comanda/balcão passaram a enfileirar pré-venda sozinhos ao fechar/pagar, ver histórico do commit), apareceram casos onde `gdoor_jobs.status` virava `processado` com `venda_id_gdoor` preenchido (Firebird aceitou o `INSERT`, `con.commit()` rodou sem exceção — confirmado lendo `criar_pre_venda` linha a linha) mas a venda não aparecia na tela de Pré-vendas do GDOOR. Não foi possível confirmar a causa raiz na sessão (não chegou a comparar `PDV_ARQBIN`/`VENDAS_NUMERO_DOC` da venda problemática contra uma de referência, nem descartar filtro de tela/cache do GDOOR) — passou a acontecer de forma intermitente mesmo com vendas de teste consecutivas (algumas apareciam, outras não), o suficiente pra não confiar no automático.

**Decisão do dono:** desligar todo disparo automático — comanda/balcão (`SalaoService.fecharComanda()` e `SalaoPdvService.pagar()`) e, na sequência, também delivery (`PedidosService.atualizarStatus`, gatilho em `delivered`) — e manter só o botão manual "Enviar para GDOOR", clicado sob demanda depois da venda já concluída, pra dar pra conferir na hora se apareceu antes de seguir pra próxima. Motivo de incluir delivery também: mesma classe de sintoma (job `processado` sem aparecer na tela), preferiu uniformizar em vez de manter automático só num dos três fluxos.

**Como apply:** se pedirem pra automatizar de novo o envio de qualquer um dos três fluxos, investigar antes essa intermitência (comparar `PDV_ARQBIN` byte a byte entre uma venda que apareceu e uma que não apareceu, checar se há alguma condição de corrida entre `PREVENDA_MOV`/`VENDAS_NUMERO_DOC` e o refresh da tela) — reativar sem entender a causa arrisca repetir o mesmo problema.
