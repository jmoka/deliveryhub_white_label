...
import { getRelatorio, getMinhaEmpresa, getCaixa, getCaixaHistorico } from '../../services/restauranteService';
...
const STATUS_LABELS   = { pending: 'Recebido', confirmed: 'Confirmado', preparing: 'Em Preparo', ready: 'Pronto', motoboy_collecting: 'Motoboy vindo', out_for_delivery: 'Em Entrega', delivered: 'Entregue', canceled: 'Cancelado' };
...
const buildRange = (modo, dia, mes, ano, ini, fim) => {
  ...
  if (modo === 'mes') {
    const [y, m] = mes.split('-');
    return { de: startOf(`${y}-${m}-01`), ate: endOf(`${y}-${m}-${String(lastDay(mes)).padStart(2,'0')}`), label: new Date(mes + '-01T12:00:00').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }) };
  }
...
const buildRelatorio = (dados, nome, label) => {
  ...
  const saidasRows = (dados.saidas ?? []).map((s) =>
    `<tr><td>${fmtDate(s.criado_em)}</td><td>${s.descricao}</td><td class="right bold red">- ${fmt(s.valor)}</td></tr>`).join('');

  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Relatório Financeiro</title><style>${style}</style></head><body>
<h1>${nome ?? 'RESTAURANTE'}</h1><div class="sub">Relatório Financeiro — ${label}</div>
<div>
  ...
  <div class="kpi"><span class="val">${r.total_pedidos}</span><span class="lbl">Pedidos</span></div>
...
<table><tr><th>Método</th><th class="right">Qtd</th><th class="right">Total</th></tr>${pgto}</table>
<h2>Canal de Recebimento</h2>
<table><tr><th>Canal</th><th class="right">Valor</th></tr>
<tr><td>Dinheiro (físico)</td><td class="right bold green">${fmt(cashTotal)}</td></tr>
<tr><td>PIX / Cartão (digital)</td><td class="right bold">${fmt(digitalTotal)}</td></tr></table>
${(dados.saidas ?? []).length > 0 ? `<h2>Saídas (${dados.saidas.length})</h2><table><tr><th>Data</th><th>Descrição</th><th class="right">Valor</th></tr>${saidasRows}</table>` : ''}
...
const RestauranteFinanceiro = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([getCaixa(), getCaixaHistorico()]).then(([c, hist]) => {
      const cofre      = c.saldo_caixa ?? 0;
      const fechados   = c.saldo_fechados_pendente ?? 0;
      const caixaAtual = c.aberto ? (c.resumo?.saldo ?? 0) : 0;
      setPosicao({ cofre, fechados_pendente: fechados, caixa_aberto: caixaAtual, aberto: c.aberto ?? false, total: cofre + fechados + caixaAtual });

      // Caixas ativos hoje (aberto hoje ou fechado hoje)
      const td = todayStr();
      const hoje = (hist.historico ?? []).filter((cx) => {
        const diaAberto  = new Date(cx.aberto_em).toISOString().slice(0, 10);
        const diaFechado = cx.fechado_em ? new Date(cx.fechado_em).toISOString().slice(0, 10) : null;
        return diaAberto === td || diaFechado === td;
      });
      setCaixasHoje(hoje);
    }).catch(() => {});
    buscar();
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
      ...
      <header className="bg-white border-b border-[#E4E4E7] px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={() => navigate('/restaurante')} className="md:hidden flex items-center gap-1.5 text-sm text-[#71717A] hover:text-[#18181B]">
      ...
      <main className="p-6 w-[95%] mx-auto max-w-5xl space-y-5">
          ...
          <div className="flex flex-wrap gap-2 items-end">
            ...
            {modo === 'periodo' && (
              <>
                ...
                <input type="date" value={fim} min={ini} onChange={(e) => setFim(e.target.value)}
                  className="border border-[#E4E4E7] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#FF441F]" />
              </>
            )}
            ...
            {dados && (
              <button onClick={() => printIframe(buildRelatorio(dados, restauranteNome, label))}
        ...
        {/* Posição de caixa — sempre visível, independente do período */}
        {posicao !== null && (
          <div className="bg-white rounded-2xl border border-[#E4E4E7] p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm font-bold text-[#18181B]">Posição do Caixa Físico</p>
                <p className="text-[10px] text-[#71717A] mt-0.5">Dinheiro presente no estabelecimento agora</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-[#71717A] uppercase tracking-widest">Total</p>
                <p className="text-2xl font-black text-[#18181B]">{fmt(posicao.total)}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {/* Cofre (saldo_caixa explícito) */}
              <div className={`rounded-xl p-3 text-center border ${posicao.cofre > 0 ? 'bg-[#FF441F]/5 border-[#FF441F]/20' : 'bg-[#F4F4F5] border-[#E4E4E7]'}`}>
                <p className="text-lg mb-1">🗃️</p>
                <p className={`text-lg font-black ${posicao.cofre > 0 ? 'text-[#FF441F]' : 'text-[#A1A1AA]'}`}>{fmt(posicao.cofre)}</p>
                <p className="text-[10px] text-[#71717A] mt-0.5 leading-tight">Cofre<br />(após fechamentos)</p>
              </div>
              {/* Caixas fechados sem destinação */}
              <div className={`rounded-xl p-3 text-center border ${posicao.fechados_pendente > 0 ? 'bg-amber-50 border-amber-200' : 'bg-[#F4F4F5] border-[#E4E4E7]'}`}>
                <p className="text-lg mb-1">📦</p>
                <p className={`text-lg font-black ${posicao.fechados_pendente > 0 ? 'text-amber-700' : 'text-[#A1A1AA]'}`}>{fmt(posicao.fechados_pendente)}</p>
                <p className="text-[10px] text-[#71717A] mt-0.5 leading-tight">Caixas fechados<br />(sem destinação)</p>
              </div>
              {/* Caixa aberto atual */}
              <div className={`rounded-xl p-3 text-center border ${posicao.aberto ? 'bg-green-50 border-green-200' : 'bg-[#F4F4F5] border-[#E4E4E7]'}`}>
                <p className="text-lg mb-1">{posicao.aberto ? '🟢' : '⚪'}</p>
                <p className={`text-lg font-black ${posicao.aberto ? 'text-green-700' : 'text-[#A1A1AA]'}`}>{fmt(posicao.caixa_aberto)}</p>
                <p className="text-[10px] text-[#71717A] mt-0.5 leading-tight">Caixa aberto<br />{posicao.aberto ? 'em operação' : 'fechado'}</p>
              </div>
            </div>
            {posicao.fechados_pendente > 0 && (
              <p className="text-[10px] text-amber-600 mt-3 flex items-start gap-1">
                <span>⚠️</span>
                <span>{fmt(posicao.fechados_pendente)} em caixas fechados antes da funcionalidade de destinação. Feche e reabra esses caixas para regularizar.</span>
              </p>
            )}
            ...
            {caixasHoje.length > 0 && (
              <div className="mt-4 border-t border-[#F4F4F5] pt-4">
                <p className="text-[10px] font-black text-[#A1A1AA] uppercase tracking-widest mb-2">
                  Caixas de hoje ({caixasHoje.length})
                </p>
                <div className="space-y-2">
                  {caixasHoje.map((cx) => {
                    const r = cx.resumo ?? {};
                    const isAberto  = cx.status === 'aberto';
                    const isExpired = cx.status === 'expirado';
                    const fmtHora   = (d) => d ? new Date(d).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '—';
                    return (
                      <div key={cx.id} className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm border ${isAberto ? 'bg-green-50 border-green-200' : isExpired ? 'bg-red-50 border-red-200' : 'bg-[#FAFAFA] border-[#E4E4E7]'}`}>
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-base flex-shrink-0">{isAberto ? '🟢' : isExpired ? '🔴' : '⚫'}</span>
                          <div className="min-w-0">
                            <p className="font-semibold text-[#18181B] truncate">{cx.nome_operador}</p>
                            <p className="text-[10px] text-[#71717A]">
                              {fmtHora(cx.aberto_em)}{cx.fechado_em ? ` → ${fmtHora(cx.fechado_em)}` : ' (aberto)'}
                              {(r.total_pedidos ?? 0) > 0 && ` · ${r.total_pedidos} pedidos`}
                            </p>
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0 ml-3">
                          <p className={`font-black ${isAberto ? 'text-green-700' : 'text-[#18181B]'}`}>{fmt(r.saldo)}</p>
                          <p className="text-[10px] text-[#71717A]">vendas {fmt(r.total_vendas)}</p>
                        </div>
                      </div>
                    );
                  })}
        ...
        {dados && r && (
          <>
            ...
            <div className="flex gap-1 bg-[#F4F4F5] p-1 rounded-xl w-fit">
              {[
                { id: 'resumo',   label: 'Entradas' },
                { id: 'canais',   label: 'Canais' },
                { id: 'saidas',   label: `Saídas (${(dados.saidas ?? []).length})` },
                { id: 'pedidos',  label: `Pedidos (${dados.pedidos?.length ?? 0})` },
              ].map((a) => (
                  ...
                  Object.entries(r.por_pagamento ?? {}).map(([k, v]) => (
                    <div key={k} className={`bg-white border rounded-2xl p-4 ${PAYMENT_COLORS[k] ?? 'border-gray-200 bg-gray-50 text-gray-800'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <Icon name={PAYMENT_ICONS[k] ?? 'DollarSign'} size={16} />
                        <p className="text-xs font-bold uppercase tracking-wide">{PAYMENT_LABELS[k] ?? k}</p>
                      </div>
            ...
            {/* Aba: Canais (dinheiro físico vs digital) */}
            {abaAtiva === 'canais' && (
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-white border border-green-200 rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-bold text-[#18181B]">Dinheiro Físico (Caixa)</h3>
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-green-100">
                    <p className="text-xs text-[#71717A]">Nota: saídas de caixa podem ter saído deste valor.</p>
                    <p className="text-xs font-semibold text-green-700 mt-1">Estimativa em caixa: {fmt(Math.max(0, cashTotal - r.total_saidas))}</p>
                  </div>
                </div>
                <div className="bg-white border border-blue-200 rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Icon name="Landmark" size={18} className="text-blue-600" />
                    <h3 className="font-bold text-[#18181B]">Digital / Banco</h3>
                  </div>
                  <p className="text-3xl font-black text-blue-700 mb-1">{fmt(digitalTotal)}</p>
                  <p className="text-xs text-[#71717A]">PIX, cartão crédito e débito</p>
                  <div className="mt-4 pt-3 border-t border-blue-100 space-y-1">
                    {Object.entries(r.por_pagamento ?? {}).filter(([k]) => k !== 'cash').map(([k, v]) => (
                      <div key={k} className="flex justify-between text-xs">
                        <span className="text-[#71717A]">{PAYMENT_LABELS[k] ?? k}</span>
                        <span className="font-semibold text-[#18181B]">{fmt(v.total)} ({v.count})</span>
                      </div>
                    ))}
                    {Object.keys(r.por_pagamento ?? {}).filter((k) => k !== 'cash').length === 0 && (
                      <p className="text-xs text-[#71717A]">Nenhum pagamento digital no período.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {abaAtiva === 'saidas' && (
              <div className="bg-white rounded-2xl border border-[#E4E4E7] overflow-hidden">
                {(dados.saidas ?? []).length === 0 ? (
                  ...
                  <div>
                    <div className="px-5 py-3 bg-[#FAFAFA] border-b border-[#F4F4F5] flex justify-between text-xs font-bold text-[#71717A] uppercase tracking-widest">
                    ...
                    <div className="divide-y divide-[#F4F4F5] max-h-[400px] overflow-y-auto">
                      {(dados.saidas ?? []).sort((a, b) => new Date(b.criado_em) - new Date(a.criado_em)).map((s, i) => (
                        <div key={i} className="flex items-center justify-between px-5 py-3">
                          <div>
                            <p className="text-sm font-semibold text-[#18181B]">{s.descricao}</p>
                            <p className="text-xs text-[#71717A] flex items-center gap-1.5">
                              {fmtDate(s.criado_em)}
                              {s.meio && <span className="px-1.5 py-0.5 bg-[#F4F4F5] rounded text-[10px] font-semibold capitalize">{s.meio}</span>}
                            </p>
                          </div>
                    ...
                    <div className="px-5 py-3 bg-red-50 border-t border-red-100 flex justify-between">
                      <span className="text-sm font-bold text-red-700">Total saídas</span>
                      <span className="text-sm font-black text-red-700">- {fmt(r.total_saidas)}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {abaAtiva === 'pedidos' && (
              <div className="bg-white rounded-2xl border border-[#E4E4E7] overflow-hidden">
                {(dados.pedidos ?? []).length === 0 ? (
                  ...
                  <div>
                    <div className="px-5 py-3 bg-[#FAFAFA] border-b border-[#F4F4F5] grid grid-cols-4 text-xs font-bold text-[#71717A] uppercase tracking-widest">
                      <span>Pedido</span>
                      <span>Cliente</span>
                      <span>Pagamento</span>
                          ...
                          <div key={p.id} className="px-5 py-3 grid grid-cols-4 items-center gap-2 hover:bg-[#FAFAFA]">
                            <div>
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <span className="text-sm font-bold text-[#18181B]">#{p.id}</span>
                                <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-semibold ${sc}`}>{STATUS_LABELS[p.status] ?? p.status}</span>
                              </div>
                              <p className="text-[10px] text-[#71717A]">{fmtDate(p.created_at)}</p>
                            </div>
                            <p className="text-xs text-[#27272A] truncate">{p.customers?.name ?? '—'}</p>
                    ...
                    <div className="px-5 py-3 bg-[#FAFAFA] border-t border-[#E4E4E7] flex justify-between">
                      <span className="text-sm text-[#71717A]">{dados.pedidos.length} pedidos · {r.cancelados} cancelados</span>
                      <span className="text-sm font-black text-green-700">{fmt(r.total_vendas)} faturados</span>
                    </div>
                  </div>
                )}
              </div>
