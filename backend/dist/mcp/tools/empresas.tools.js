...
exports.empresasToolDefinitions = void 0;
exports.executarEmpresasTool = executarEmpresasTool;
exports.empresasToolDefinitions = [
    {
...
async function executarEmpresasTool(nome, args, supabase) {
    switch (nome) {
...
async function listarEmpresas(args, supabase) {
    let query = supabase
        .from('restaurants')
...
async function buscarEmpresa(args, supabase) {
...
async function listarComissoes(args, supabase) {
    let query = supabase
        .from('plataforma_comissoes')
        .select('id, empresa_id, pedido_id, valor_venda, comissao_pct, comissao_valor, criado_em')
        .order('criado_em', { ascending: false });
    if (args.empresa_id)
        query = query.eq('empresa_id', args.empresa_id);
    if (args.data_inicio)
        query = query.gte('criado_em', args.data_inicio);
    if (args.data_fim)
        query = query.lte('criado_em', args.data_fim + 'T23:59:59');
    const { data, error } = await query;
    if (error)
        throw error;
...
//# sourceMappingURL=empresas.tools.js.map