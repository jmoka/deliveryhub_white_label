...
exports.pedidosToolDefinitions = void 0;
exports.executarPedidosTool = executarPedidosTool;
exports.pedidosToolDefinitions = [
    {
        name: 'listar_pedidos',
...
async function executarPedidosTool(nome, args, supabase) {
    switch (nome) {
...
async function listarPedidos(args, supabase) {
    let query = supabase
        .from('orders')
    ...
    if (args.data_inicio)
        query = query.gte('created_at', args.data_inicio);
    if (args.data_fim)
        query = query.lte('created_at', args.data_fim + 'T23:59:59');
    const { data, error } = await query;
    if (error)
        throw error;
    return { pedidos: data, total: data?.length ?? 0 };
}
async function buscarPedido(args, supabase) {
...
async function estatisticasPedidos(args, supabase) {
    let query = supabase
        .from('orders')
...
//# sourceMappingURL=pedidos.tools.js.map